...
id_list		: TOK_VARIABLE
			{
				if(ts != NULL)
				{
					if(ts->exists($1) == 1)
					{
						sprintf(msg,"%d:%d Semantic Err! %s already declared!", @1.first_line, @1.first_column, $1);
						yyerror(msg);
						YYERROR;
					}
					else
		...
		id_list ',' TOK_VARIABLE
			{
				if(ts != NULL)
				{
					if(ts->exists($3) == 1)
					{
						sprintf(msg,"%d:%d Semantic Err! %s already declared!", @1.first_line, @1.first_column, $3);
						yyerror(msg);
						YYERROR;
					}
					else
...
assign		: TOK_VARIABLE TOK_ASSIGN exp 
			{
				if(ts != NULL)
				{
					if(ts->exists($1) != 1)
					{
						sprintf(msg,"%d:%d Semantic Err! %s not declared!", @1.first_line, @1.first_column, $1);
						yyerror(msg);
						YYERROR;
					}
				}
		...
		id_list_for_rw ',' TOK_VARIABLE
			{
				if(ts != NULL)
				{
					if(ts->exists($3) != 1)
					{
						sprintf(msg,"%d:%d Semantic Err! %s not declared!", @1.first_line, @1.first_column, $3);
						yyerror(msg);
						YYERROR;
					}
				}
...
int yyerror(const char *message) {
	...
	cout << "Line" << LineNo << " Col" << ColNo << endl;
